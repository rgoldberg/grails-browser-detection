...
class BrowserTagLib {
	...
	private static final String CHOICE_STACK_NAME = "${this.name}_choiceStack"
	...
	private handleBrowser(Map attrs, Closure body, String serviceMethodName) {
		def version
		def comparisonType
		if (attrs.version) {
		...
		handle body, {
			userAgentIdentService."$serviceMethodName"(comparisonType, version)
		}
	}

	/**
	 * @param body a tag body object
	 * @param condition a closure that returns true or false
	 * @return true if
	 */
	private boolean handle(Closure body, Closure condition) {
		def stack = getStack()
		def parent = (!stack.empty()) ? stack.peek() : null

		// skip if successful condition is met in choice tag
		if (parent && parent.levelType == HierarchyLevelType.ChoiceTag && parent.successfulCondition) {
			return
		}

		if (condition()) {
			stack.push new HierarchyLevelHolder(levelType: HierarchyLevelType.ConditionTag)
			...
			if (parent?.levelType == HierarchyLevelType.ChoiceTag) {
	...
	/**
	 * Returns hierarchy stack, Creates one if it does not exist yet.
	 */
	private Stack<HierarchyLevelHolder> getStack() {
		def stack = request."$CHOICE_STACK_NAME"

		if (stack == null) {
			request."$CHOICE_STACK_NAME" = stack = new Stack()
		}
